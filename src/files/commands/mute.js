module.exports.run = async (prefix, client, message, args) => {
	if (!message.member.permissions.has('MANAGE_ROLES')) {message.channel.send("You don't have permission to mute this user!");}
	const member = message.mentions.members.first();
	if (!member) return message.channel.send('You must mention an user!');
	if (member.permissions.has('ADMINISTRATOR')) {message.channel.send("This user is an administrator, I can't mute him!");}
	if (!member.manageable) {return message.channel.send("I don't have permission to mute this user!");}
	let role = undefined;
